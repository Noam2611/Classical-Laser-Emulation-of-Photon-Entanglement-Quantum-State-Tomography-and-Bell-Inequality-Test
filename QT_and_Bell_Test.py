...
EVENT_THRESHOLD = 0.89         # 0..1, fraction of normalised intensity
MIN_EVENT_SEPARATION = 30      # minimum frame gap between two distinct events

COINCIDENCE_WINDOW = 12       # ± frames searched in channel B around each A event
...
def plot_rho_3d(ax, rho, title, n_layers=48):
    """3-D bar chart of |rho_ij| with a vertical colour gradient on each bar and
    flat base squares (no tile/bar overlap). Colour maps to
    absolute height, anchored at 0 and shared across bars."""
    Z = np.abs(rho)
    ...
    zmax = float(np.max(dz)) if np.max(dz) > 0 else 1e-12
    norm = mcolors.Normalize(vmin=0.0, vmax=zmax)
    cmap = cm.coolwarm
    tile_thresh = 1e-3 * zmax   # cells below this count as "empty"

    for xi, yi, h in zip(x, y, dz):
        if h <= tile_thresh:
            # flat base square only where there is no bar -> no overlap
            quad = [[(xi, yi, 0), (xi + dx, yi, 0),
                     (xi + dx, yi + dy, 0), (xi, yi + dy, 0)]]
            tile = Poly3DCollection(quad, facecolors=cmap(0.0),
                                    edgecolors="0.6", linewidths=0.3)
            ax.add_collection3d(tile)
        else:
            edges = np.linspace(0.0, h, n_layers + 1)
            for z0, z1 in zip(edges[:-1], edges[1:]):
                ax.bar3d(xi, yi, z0, dx, dy, z1 - z0,
                         color=cmap(norm(0.5 * (z0 + z1))), shade=False)
...
def run_part_b():
    ...
    for i, alpha in enumerate(ALPHA_ANGLES):
        for j, beta in enumerate(BETA_ANGLES):
            ...
            ax.axhline(EVENT_THRESHOLD, ls="--", color="black", alpha=0.6)
